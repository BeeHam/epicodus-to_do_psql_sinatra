...
describe(List) do
  ...
  describe('#tasks') do
    it("returns an array of tasks for that list") do
      test_list = List.new({:name => "Epicodus stuff", :id => nil})
      test_list.save()
      test_task = Task.new({:description => "Learn SQL", :list_id => test_list.id(), :due => '2015-08-19', :id => nil})
      test_task.save()
      test_task2 = Task.new({:description => "Review Ruby", :list_id => test_list.id(), :due => '2015-08-19', :id => nil})
      test_task2.save()
